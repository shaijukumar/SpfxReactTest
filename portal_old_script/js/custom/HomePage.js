...
function UpdateUserProfile($scope) {
  ...
  UserProfile.MyDashbord =
    window.location.origin + "/ar-ae/Pages/MyProfile.aspx";
  if (!UserProfile.PictureUrl)
    UserProfile.PictureUrl =
      "/_catalogs/masterpage/GSMOPA/images/default-user.svg";

  getItemsByREST(
    null,
    null,
    _spPageContextInfo.siteAbsoluteUrl +
      "/en-us/_api/web/lists/getbytitle('Variation')/items?" +
      "$filter=VariationGroup eq 'UserProfile'&$select=Arabic,Title&$OrderBy=Ordering"
        ...
        $.each(data.d.results, function (key, ans) {
          ...
          if (ans.AttachmentFiles.results.length > 0) {
            anns.PublishingPageImage =
              ans.AttachmentFiles.results[0].ServerRelativeUrl;
          } else {
            anns.PublishingPageImage = "";
          }
...
app.controller("policyController", function ($scope, $http) {
  ...
  var filterPolicy =
    "$select=OData__UIVersionString,ID,UniqueId,PolicyDescription,Policy_x0020_Name1,Department,Modified,EncodedAbsUrl&$orderby=Modified desc&$Top=9";
  getItemsByREST("PoliciesDetails", filterPolicy).then(function (restItems) {
    var PolicyList = restItems;
    var PolicyArray = [];
    while (PolicyList.length > 0) PolicyArray.push(PolicyList.splice(0, 3));
    $scope.policiesData = PolicyArray;
  });
  $scope.SelectPolicy = function (policy) {
    $scope.PolicyData = policy;
    $scope.PolicyData.IframeUrl =
      _spPageContextInfo.webAbsoluteUrl +
      "/_layouts/15/WopiFrame.aspx?sourcedoc={" +
      policy.UniqueId +
      "}&action=embedview&copy=false";
...
function displayCEODetails($scope) {
  $.ajax({
    url:
      _spPageContextInfo.webAbsoluteUrl +
      "/_api/web/lists/getbytitle('CEO')/items?$top=1",
    type: "GET",
    headers: {
